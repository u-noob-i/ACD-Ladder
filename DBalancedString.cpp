...
void solve()
{
    ...
    cin >> s;
    int n = s.length(), o = 0;
    rep(i, 0, n - 1) if (s[i] == '0') o++;
    vvi dp(n + 1, vi(n * n + 1, n));
    dp[0][0] = 0;
    rep(i, 0, n - 1)
    {
        vvi t(n + 1, vi(n * n + 1, n));
        rep(j, 0, i) rep(k, 0, j * (i - j))
        {
            t[j + 1][k] = min(t[j + 1][k], dp[j][k] + (s[i] == '1'));
            t[j][k + j] = min(t[j][k + j], dp[j][k] + (s[i] == '0'));
        }
        swap(dp, t);
    }
    cout << dp[o][(o * (n - o)) / 2] / 2;
