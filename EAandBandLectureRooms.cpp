...
vi adj[100005];
int sz[100005], tin[100005], tout[100005];
int lift[100005][25];
int tmr;
...
void solve()
{
    ...
    rep(i, 1, m)
    {
        ...
        u--, v--;
        int l = lca(u, v);
        int x = getdist(u, l), y = getdist(v, l), ans;
        if ((x + y) % 2)
            ans = 0;
        else if (y > x)
            ans = sz[getkparent(v, (x + y) / 2)] - sz[getkparent(v, (x + y) / 2 - 1)];
        else if (x > y)
            ans = sz[getkparent(u, (x + y) / 2)] - sz[getkparent(u, (x + y) / 2 - 1)];
        else
            ans = n - sz[getkparent(u, x - 1)] - sz[getkparent(v, y - 1)];
        cout << ans << "\n";
    }
}
